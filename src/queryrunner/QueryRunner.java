...
public class QueryRunner {
	...
	/**
	 * Recent Skill Updates For Staff Working on Projects for a Manager Shows the
	 * most recent skill updates for each staff working on the projects managed by
	 * certain project manager. Note: The query first identified the staff working
	 * on tasks under deliverables that belong to each project managed by Aarika
	 * Paike. Then the query determined the most recent skill acquired by each staff
	 * among their other skills.
	 */
	private void recentSkillUpdatesebyPMQuery() {
		// queryArray[4]
		// recent skill updates for employees under PM
		queryArray.add(new QueryData("   SELECT" + "    Tasks.task_owner_user_id AS project_employee_id,"
				+ "    Users_has_Skills.date_skill_acquired," + "    Users_has_Skills.skill_id,"
				+ "    Skills.skill_name" + "  FROM Projects," + "  Deliverables," + "  Tasks," + "  Users_has_Skills,"
				+ "  Skills" + "  WHERE" + "    Skills.skill_id = Users_has_Skills.skill_id"
				+ "    AND Projects.project_id = Deliverables.project_id"
				+ "    AND Deliverables.deliverable_id = Tasks.deliverable_id" + "    AND Projects.project_manager_id ="
				+ "     (SELECT Users.user_ID FROM Users" + "     WHERE" + "     (Users.user_last_name LIKE ?) )"
				+ "     AND Users_has_Skills.date_skill_acquired ="
				+ "       (SELECT MAX(Users_has_Skills.date_skill_acquired)" + "       FROM Users_has_Skills"
				+ "       WHERE" + "       Tasks.task_owner_user_id = Users_has_Skills.user_id)"
				+ "  GROUP BY Tasks.task_owner_user_id;", new String[] { "PM First Name" }, new boolean[] { true },
				false, true));
	}
	...
	private void IncompleteTasksQuery() {
		...
		queryArray.add(new QueryData("SELECT task_name as 'Task',   "
				...
				+ "	 project_end_date as 'Project Ends'," + "  blockers_flagged as 'Blockers Flagged',"
				+ "  deliverable_name as 'Deliverable', " + "  project_name as 'Project', project_manager as 'Manager'"
				+ "  FROM IncompleteTasks_ProjectsEndingThisMonth", null, null, false, false));
	}
	...
	private void EmployeeTasksQuery() {
		// queryArray[6]
		queryArray.add(new QueryData("SELECT " + "task_name as 'Task', end_time as 'Finish Date', "
				+ "expected_time_to_complete as 'Excpected Completion Time', "
				+ "real_time_to_complete 'Real Completion Time', " + "blockers_flagged as 'Blockers', "
				+ "difficulty_rate_id as 'Difficulty Rating'," + "employee as 'Employee'"
				+ " FROM Employee_TasksOrderByEndDate ", null, null, false, false));
		// queryArray[7]
		queryArray.add(new QueryData(
				"SELECT " + "task_name as 'Task', end_time as 'Finish Date', "
						+ "expected_time_to_complete as 'Excpected Completion Time', "
						+ "real_time_to_complete 'Real Completion Time', " + "blockers_flagged as 'Blockers', "
						+ "difficulty_rate_id as 'Difficulty Rating'," + "employee as 'Employee'"
						+ " FROM Employee_TasksOrderByEndDate " + "WHERE employee LIKE ?",
				new String[] { "First Name" }, new boolean[] { true }, false, true));
	}
	...
	private void InsertTaskQuery() {
		// queryArray[8]
		queryArray.add(new QueryData("INSERT INTO Tasks (task_name, deliverable_id, task_owner_user_id) VALUES (?,?,?)",
				new String[] { "Task Name", "Deliverable ID", "User ID" }, new boolean[] { false, false, false }, true,
				true));
	}
	...
	public static void main(String[] args) {
		...
		java.awt.EventQueue.invokeLater(new Runnable() {
			public void run() {
				new ClincalDataFrontEnd(queryrunner).setVisible(true);
			}
		});
	}
}
